...
def integral(m):
    ...
    if ((type(a).__name__ != "int" and type(a).__name__ != "float") or (type(b).__name__ != "int" and type(b).__name__ != "float")):
        bot.send_message(cid, "Los límites de integración han de ser números reales")
        return
    Sfuncion = text[j+1:]
...
#    bot.send_video(cid, video)
    bot.send_message (cid, "¡¡Feliz Navidad!!\n¡Que Bernier y Galindo os regalen muchos aprobados, los vais a nesecitar!")

@bot.message_handler(func=lambda message: message.content_type == "text" and "buenos dias" in message.text.lower())
def buenos_dias(m):
	cid = m.chat.id
...
@bot.message_handler(func=lambda message: message.content_type == "text" and ("pole" in message.text.lower() or "polo" in message.text.lower()))
def Pole(m):
    bot.reply_to(m, "Marco!")
    bot.reply_to(m, "Plata")

@bot.message_handler(func=lambda message: message.content_type == "text" and "plata" in message.text.lower())
def Plata(m):
    bot.reply_to(m, "Plata de plátano")
    bot.reply_to(m, "Fail")

@bot.message_handler(func=lambda message: message.content_type == "text" and "fail" in message.text.lower())
def Fail(m):
    bot.reply_to(m, "Fail, la nota que vas a tener en Mécanica")
